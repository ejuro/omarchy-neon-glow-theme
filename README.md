...
Already installed through the Omarchy theme menu? Run just:
...
A small user service watches the selected wallpaper, applies its palette, and uses Omarchy's own refresh commands. It only acts while Neon Glow is selected. Changing to another theme works normally. Custom wallpapers without a palette mapping retain the current colors.
